...
class ArrayDeque[A] private[ArrayDeque](
  ...
  /**
    ...
    * @return
    */
  @`inline` final def peek: Option[A] = headOption
  ...
  def iterableFactory: SeqFactory[ArrayDeque] = ArrayDeque
  ...
  override def copyToArray[B >: A](dest: Array[B], destStart: Int, len: Int): Unit =
...
class ArrayDequeView[A](val parent: ArrayDeque[A]) extends IndexedView[A] {
  ...
  def length: Int = parent.size

  def apply(idx: Int) = parent(idx)
